...
export const grantManagementRouter = router({
  ...
  // Get suggested staffing bundles for a grant opportunity
  getSuggestedBundles: protectedProcedure
    .input(z.object({ grantId: z.number() }))
    .query(async ({ input }) => {
      const db = await getDb();
      if (!db) throw new Error("Database not available");
      
      const [grant] = await db.select().from(grantOpportunities).where(eq(grantOpportunities.id, input.grantId));
      if (!grant) return [];
      
      const focusAreas = grant.focusAreas || [];
      const funderType = grant.funderType;
      
      // Bundle definitions with matching criteria
      const bundles = [
        {
          id: "workforce-dev",
          name: "Workforce Development",
          matchScore: 0,
          keywords: ["workforce", "employment", "job", "training", "career", "skills"],
          funderTypes: ["federal", "state"],
        ...
        },
        {
          ...
          matchScore: 0,
          keywords: ["health", "wellness", "community", "social", "prevention", "care"],
          funderTypes: ["federal", "foundation"],
        ...
        },
        {
          ...
          matchScore: 0,
          keywords: ["media", "outreach", "communication", "engagement", "content", "arts"],
          funderTypes: ["foundation", "corporate"],
        ...
        },
        {
          ...
          matchScore: 0,
          keywords: ["capacity", "operations", "administrative", "organizational", "infrastructure"],
          funderTypes: ["foundation", "corporate"],
        ...
        },
        {
          ...
          matchScore: 0,
          keywords: ["education", "learning", "curriculum", "teaching", "academic", "student"],
          funderTypes: ["federal", "foundation", "state"],
          positionIds: ["EOC-001", "DOC-001", "PA-001", "BSS1-001"],
        },
      ];
      
      // Score bundles based on matching
      ...
      for (const bundle of bundles) {
        ...
        // Funder type matching
        if (bundle.funderTypes.includes(funderType)) {
          bundle.matchScore += 5;
        }
      }
      
      // Return top 3 bundles sorted by score
      return bundles
