 ...
 */
export const systemDiagnostics = mysqlTable("system_diagnostics", {
  id: int("id").autoincrement().primaryKey(),
  category: mysqlEnum("category", ["database", "api", "entity", "token", "compliance", "security", "performance", "integration"]).notNull(),
  checkName: varchar("check_name", { length: 100 }).notNull(),
  status: mysqlEnum("status", ["healthy", "warning", "critical", "unknown"]).notNull().default("unknown"),
...
/**
 * Diagnostic Runs - Scheduled and manual diagnostic runs
 */
export const diagnosticRuns = mysqlTable("diagnostic_runs", {
  id: int("id").autoincrement().primaryKey(),
  runType: mysqlEnum("run_type", ["scheduled", "manual"]).notNull(),
  triggeredBy: varchar("triggered_by", { length: 255 }),
  status: mysqlEnum("status", ["running", "completed", "failed"]).notNull().default("running"),
  totalChecks: int("total_checks").default(0),
  passedChecks: int("passed_checks").default(0),
  warningChecks: int("warning_checks").default(0),
...
export type InsertDiagnosticRun = typeof diagnosticRuns.$inferInsert;

/**
 * Self Corrections - Auto-fix history
 */
export const selfCorrections = mysqlTable("self_corrections", {
  id: int("id").autoincrement().primaryKey(),
  diagnosticRunId: int("diagnostic_run_id"),
  issueType: varchar("issue_type", { length: 100 }).notNull(),
  issueDescription: text("issue_description").notNull(),
  correctionAction: varchar("correction_action", { length: 255 }).notNull(),
  correctionDetails: json("correction_details"),
  status: mysqlEnum("status", ["pending", "applied", "failed", "rolled_back"]).notNull().default("pending"),
...
export type SelfCorrection = typeof selfCorrections.$inferSelect;
export type InsertSelfCorrection = typeof selfCorrections.$inferInsert;

/**
 * Diagnostic Schedules - Configurable diagnostic intervals
 */
export const diagnosticSchedules = mysqlTable("diagnostic_schedules", {
  id: int("id").autoincrement().primaryKey(),
  name: varchar("name", { length: 100 }).notNull(),
  ...
  notifyOnWarning: boolean("notify_on_warning").default(false),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow().onUpdateNow(),
});

export type DiagnosticSchedule = typeof diagnosticSchedules.$inferSelect;
export type InsertDiagnosticSchedule = typeof diagnosticSchedules.$inferInsert;
