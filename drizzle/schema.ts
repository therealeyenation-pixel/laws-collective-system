import { int, mysqlEnum, mysqlTable, text, timestamp, varchar, decimal, json, boolean, bigint, date, index, unique } from "drizzle-orm/mysql-core";
...
/**
 * Purchased Course Progress - Track user progress through purchased course lessons
 */
export const purchasedCourseProgress = mysqlTable("purchased_course_progress", {
  id: int("id").autoincrement().primaryKey(),
...
export type PurchasedCourseProgressRecord = typeof purchasedCourseProgress.$inferSelect;
export type InsertPurchasedCourseProgress = typeof purchasedCourseProgress.$inferInsert;

/**
 * Course Completions - Track when users complete entire courses
 */
export const courseCompletions = mysqlTable("course_completions", {
  id: int("id").autoincrement().primaryKey(),
  ...
  // Upsell tracking
  upsellOffered: boolean("upsellOffered").default(false).notNull(),
  upsellAccepted: boolean("upsellAccepted").default(false).notNull(),
  upsellProductId: varchar("upsellProductId", { length: 100 }),
});
...
export const broadcastEpisodes = mysqlTable("broadcast_episodes", {
  ...
  audioFormat: varchar("audioFormat", { length: 20 }).default("mp3"),
  audioDuration: int("audioDuration").notNull(),
  audioFileSize: int("audioFileSize"),
  ...
  adBreakPositions: json("adBreakPositions"),
...
export const broadcastListeners = mysqlTable("broadcast_listeners", {
  ...
  totalListeningTime: int("totalListeningTime").default(0),
...
export const episodeInteractions = mysqlTable("episode_interactions", {
  ...
  userId: int("userId").notNull(),
  listeningTime: int("listeningTime").default(0),
...
export const marketData = mysqlTable("market_data", {
  ...
  dividendYield: decimal("dividendYield", { precision: 8, scale: 2 }),
  volume: bigint("volume", { mode: "number" }),
  averageVolume: bigint("averageVolume", { mode: "number" }),
